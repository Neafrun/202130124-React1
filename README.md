...
#### 새로운 React 앱 만들기
1. Expo
- Expo는 네이티브 UI를 사용하여 만든 안드로이드, ios 앱을 위한 범용 앱을 만들 수 있는 React 프레임 워크.
- 네이티브 부분을 쉽게 사용할 수 있게 해주는 React Native SDK를 제공.
- Expo는 Expo(the company)에서 유지 관리 함.
- Expo앱을 빌드하는 것은 무료이고, 구글이나 애플 스토어에 제한 없이 제출 할 수 있습니다.
...
#### 기존 프레임워크를 사용하지 않고...
1. 앱에 기존 프레임워크에서 잘 제공하지 않는 제약 조건이 있거나, 자체 프레임워크를 구축하는 것을 선호하거나, React 앱의 기본 사항을 배우려는 경우는 React 프로젝트를 처음부터 시작하는데 사용할 수 있는 다른 옵션이 있다.
2. 처음부터 시작하면 더 많은 유연성을 얻을 수 있지만 라우팅, 데이터 가져오기 및 기타 일반적인 사용 패턴에 사용할 도구를 선택해야 합니다.
3. 이미 존재하는 프레임워크를 사용하는 대신 자신만의 프레임워크를 구축하는 것과 비슷. 권장하는 프레임워크에는 이러한 문제에 대한 기본 제공 솔루션이 있음.
4. 자신만의 솔루션을 구축하려면, Vite, Parcel 또는 RSbuil와 같은 빌드 도구로 시작할 수 있도록 하는 처음부터 React 앱 만들기 가이드 참조.
